...
/* ULTRA-FAST SHA256 FOR MINING - OPTIMIZED FOR 80-BYTE BLOCKS */
...
sha256(const unsigned char *data, size_t len, unsigned char out32[32]) {
    ...
    /* APPEND BIT LENGTH */
    uint64_t bitlen = (uint64_t)(data + (ptr - data)) << 3;
    for (int i = 0; i < 8; i++) {
