...
/* Prepare registers for SIMD Salsa20 */
...
/* XOR-then-Salsa20/8-then-add */
static inline void salsa20_xor(const salsa20_blk_t *inp, salsa20_blk_t *st) {
    DECL_X; LOAD_X(*st);
    X0=_mm_xor_si128(X0,inp->q[0]);X1=_mm_xor_si128(X1,inp->q[1]);
    X2=_mm_xor_si128(X2,inp->q[2]);X3=_mm_xor_si128(X3,inp->q[3]);
    SALSA20_8ROUNDS;
    X0=_mm_add_epi32(X0,inp->q[0]);X1=_mm_add_epi32(X1,inp->q[1]);
    X2=_mm_add_epi32(X2,inp->q[2]);X3=_mm_add_epi32(X3,inp->q[3]);
    SAVE_X(*st);
}

/* blockmix_salsa for r=general */
...
static inline uint32_t blockmix_salsa_xor(const salsa20_blk_t *B1,
                                          const salsa20_blk_t *B2,
                                          salsa20_blk_t *Bout)
{
    salsa20_blk_t T = B1[1];
                  ...
                  salsa20_blk_t *V, salsa20_blk_t *XY, void *ctx)
{
    ...
    for (size_t i = 0; i < 2*r; i++) {
        uint32_t *src = (uint32_t*)(B + 64*i);
        for (int k = 0; k < 16; k++)
            X[i].w[k] = le32dec(&src[k]);
        salsa20_simd_shuffle(&X[i], &X[i]);
    }

    /* First blockmix passes */
    blockmix_salsa(X, Y);
    blockmix_salsa(Y, X);
    ...
    for (uint32_t n = 2; n < N; n <<= 1) {
        blockmix_salsa_xor(&V[j* s], &V[j* s + s], X);
        j = integerify(X, r) & (n - 1);
    }

    /* Write out */
    for (size_t i = 0; i < 2*r; i++)
        salsa20_simd_unshuffle(&X[i], (salsa20_blk_t*)(B + 64*i));
}

/* smix2: second phase of SMix_r */
                  ...
                  salsa20_blk_t *V, salsa20_blk_t *XY, void *ctx)
{
    ...
    /* Nloop must be even */
    while (Nloop > 1) {
        blockmix_salsa_xor(&V[j*s], &X[0], Y);
        blockmix_salsa_xor(&V[j*s], &X[1], Y+1);
        X[0] = Y[0];
        X[1] = Y[1];
        j = integerify(X, r) & (N - 1);
        Nloop -= 2;
             ...
             yespower_binary_t *dst)
{
    ...
    if ((version != YESPOWER_0_5 && version != YESPOWER_1_0) ||
        N < 1024 || N > 512*1024 ||
        r < 1 || r > 32 || (N & (N-1)) != 0 ||
        (version == YESPOWER_1_0 && !pers && perslen)) {
        errno = EINVAL;
        goto fail;
    }

    size_t Bsize   = 128 * r;
    size_t Vsize   = Bsize * N;
    size_t XYsize  = (version == YESPOWER_0_5 ? 2*Bsize : Bsize + 64);
    int Swidth     = (version == YESPOWER_0_5 ? 8 : 11);
    size_t Sbytes  = (version == YESPOWER_0_5
                      ? 2*((1<<Swidth)*2*8)
                      : 3*((1<<Swidth)*2*8));
    ...
    salsa20_blk_t *V = (salsa20_blk_t*)(B + Bsize);
    salsa20_blk_t *XY= (salsa20_blk_t*)(B + Bsize + Vsize);
    uint8_t *S       = (uint8_t*)(B + Bsize + Vsize + XYsize);

    uint8_t *S0 = S;
    uint8_t *S1 = S + ((1<<Swidth)*2*8);
    uint8_t *S2 = (version == YESPOWER_1_0 ? S1 + ((1<<Swidth)*2*8) : NULL);
    ...
    if (version == YESPOWER_0_5) {
        PBKDF2_SHA256(sha, 32, src, srclen, 1, B, Bsize);
        memcpy(sha, B, 32);
        smix(B, r, N, V, XY, &ctx);
        PBKDF2_SHA256(sha, 32, B, Bsize, 1, (uint8_t*)dst, sizeof(*dst));
        if (pers) {
            HMAC_SHA256_Buf(dst, sizeof(*dst), pers, perslen, sha);
            SHA256_Buf(sha, sha, dst);
        }
    } else {
        if (pers) { src = pers; srclen = perslen; }
        PBKDF2_SHA256(sha, 32, src, srclen, 1, B, 128);
        memcpy(sha, B, 32);
        smix(B, r, N, V, XY, &ctx);
        HMAC_SHA256_Buf(B + Bsize - 64, 64, sha, 32, (uint8_t*)dst);
    }
