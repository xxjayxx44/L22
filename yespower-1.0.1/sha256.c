 ...
 * optimized_sha256.c
 *
 * An optimized SHA-256 implementation tuned for Intel Celeron N4020 (SSE2),
 * including HMAC-SHA256 and PBKDF2-HMAC-SHA256.
 */
...
/* Bitwise functions via built-ins */
#define S0(x) (__builtin_rotr((x), 2)  ^ __builtin_rotr((x),13) ^ __builtin_rotr((x),22))
#define S1(x) (__builtin_rotr((x), 6)  ^ __builtin_rotr((x),11) ^ __builtin_rotr((x),25))
#define s0(x) (__builtin_rotr((x), 7)  ^ __builtin_rotr((x),18) ^ ((x) >>  3))
#define s1(x) (__builtin_rotr((x),17)  ^ __builtin_rotr((x),19) ^ ((x) >> 10))
...
/*
 * Core SHA-256 block transform: processes one 512-bit block.
 * Uses single-round loop with precomputed schedule W[0..63].
 */
static inline void
sha256_transform(uint32_t state[8], const uint8_t block[64]) __attribute__((always_inline));
static inline void
...
/*
 * Pad and process final blocks:
 * - pad with 0x80, zeros, then 64-bit big-endian length
 */
static void
SHA256_Pad(SHA256_CTX *ctx, uint8_t tmpbuf[8]) {
                   ...
                   uint64_t c, uint8_t *buf, size_t dkLen)
{
    ...
    /* Precompute inner and inner+salt ctx */
